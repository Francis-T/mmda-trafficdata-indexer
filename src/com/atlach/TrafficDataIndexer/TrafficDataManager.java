...
/**
 * <b>TrafficDataManager Class</b>
 * </br>Used for managing the update of Traffic Data from the MMDA Website
 * @author francis
 * @author keith
 *
 */
public class TrafficDataManager {
	...
	public TrafficDataManager(TrafficDataIndexerEvent evM, TrafficDataIndexerNotifier evN) {
	 ...
	 * @throws Exception
	 */
	public ArrayList<MonitoredLocation> getAllLineTrafficData() throws Exception {
		String areaName [] = {"EDSA",
							  "COMMONWEALTH",
							  "QUEZON AVE",
							  "ESPANA",
							  "C5",
	 ...
	 * @param lineViewUrl - the line view URL from which traffic data will be extracted
	 * @param areaName - the name of the area to associate with traffic data obtained 
	 * @return an integer indicating the exit status for this method
	 * @throws IOException
	 */
	private int extractLineTrafficData(String lineViewUrl, String areaName) throws IOException {
		System.out.println("extractLineTrafficData() called for " + lineViewUrl);
		eventIndexerNotif.onStatusUpdate("Downloading line view for " + lineViewUrl + "...");
        ...
        try {
	        ...
	        while ((line = rd.readLine()) != null) {
	        	if (shouldStopReading) {
	        		System.out.println("[extractLineTrafficData] Read Interrupted.");
	        		break;
	        	}
	        	
				String pattern = new String("line-name\"><p>");
				if ((line.contains(pattern) == true )&&line.contains("<a") == true) {
				...
				if ((lineCondSB != 0) &&
					...
					MonitoredLocation tempmonitoredLocList = new MonitoredLocation (lineName, 
																					areaName, 
																					lineCondSB, 
																					lineCondNB);
        	...
        	eventIndexerNotif.onStatusUpdate("Connection Timed Out for " + lineViewUrl);
        } finally {
        	if (rd != null) {
        		rd.close();
        	}
	...
	/**
	 * Saves collected traffic data to a Raw Traffic Data File. This outer method handles 
	 * exceptions resulting from the actual save operation in SaveToFile().
	 * </br>
	 * </br>The general format for this file is as follows:
	 * </br><i>[Area], [Location], [Southbound Traffic], [Northbound Traffic]</i>
	 * </br>...
	 * </br>
	 * </br>*where 1=light traffic, 2=moderate traffic, 3=heavy traffic
	 * @return
	 */
	private int saveTrafficDataToFile() {
		...
		timestamp = new SimpleDateFormat("yyyyMMdd_HH").format(Calendar.getInstance().getTime());
		int minute = 
			   (Calendar.getInstance(TimeZone.getTimeZone("GMT+8")).get(Calendar.MINUTE) / 15) * 15;
		...
		String hourTime = 
				new SimpleDateFormat("HH")
							.format(Calendar.getInstance(TimeZone.getTimeZone("GMT+8")).getTime());
		
		eventIndexerNotif.onStatusUpdate("File Saved (" + timestamp + minuteSection + 
																	"_TrafficRec.txt).");
		eventIndexerNotif.onTrafficDataFileSaved("mmda_traffic.txt", (hourTime + "00"));
		
		return STATUS_OK;
	}
	
	/**
	 * Saves collected traffic data to a Raw Traffic Data File. This inner method performs the
	 * actual save operation
	 * @param saveFileString - the name of the save file target
	 * @param locList - an ArrayList of MonitoredLocation objects
	 * @throws IOException
	 */
	private void saveToFile(String saveFileString, 
		...
		try {
			fileWriter = Files.newBufferedWriter(saveFile, Charset.forName("UTF-8"), StandardOpenOption.WRITE);
	...
	/*****************************************************************************************/
	/**
	 * <b>MonitoredLocation Object</b>
	 * </br>Represents locations which the MMDA has Traffic Data for
	 * @author francis
	 *
	 */
	public class MonitoredLocation {
		...
		public String generateWriteableString() {
			return ("" + area + ", " + name + ", " + conditionSB + ", " + conditionNB + ", " );
		}
	}
}
