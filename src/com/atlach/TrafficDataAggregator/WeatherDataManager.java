...
/**
 * <b>WeatherDataManager Class</b>
 * </br>Handles all interactions with the Web-based Weather Data provider (WorldWeatherOnline.com) 
 * @author francis
 *
 */
public class WeatherDataManager {
	...
	private static final String KEY_VAL = "XXXXXXXXXXXXXXXXXXXXXXXXX";
	 ...
	 * @param location - the location for which weather data should be obtained 
	 * @param numOfDays - the number of days of predicted weather data to obtain
	...
	public String sendWeatherDataRequest(String location, int numOfDays) {
		...
		try {
			HttpURLConnection urlConn = getServerConnection("q=" + 
															location +
															"&format=json&num_of_days=" + 
															Integer.toString(numOfDays) + 
															"&key=" + 
															this.KEY_VAL);
	 ...
	 * @return a string containing a description of the current weather condition in tag format
	 */
	public String extractCurrentWeatherCondition(String jsonString) {
		...
		try {
			...
			if (currentCond.length() != 1) {
				System.out.println("[extractCurrentWeatherCondition] Invalid length!");
				return condStr;
			}
			JSONObject currentCondChild = currentCond.getJSONObject(0);
			
			JSONArray weatherDesc = currentCondChild.getJSONArray("weatherDesc");
			...
			condStr = getSimplifiedWeatherCond(weatherDescChild.getString("value").toLowerCase());
	 ...
	 * @param jsonCondStr - the weather condition string extracted from the JSON response
	 * @return a string containing a description of the current weather condition in tag format
	 */
	private String getSimplifiedWeatherCond(String jsonCondStr) {
		String condStr = jsonCondStr;
		
		if (jsonCondStr.contains("cloudy") || jsonCondStr.contains("overcast") || jsonCondStr.contains("mist") || jsonCondStr.contains("fog")) {
			condStr = "Overcast";
		...
		if (jsonCondStr.contains("snow") || jsonCondStr.contains("ice") || jsonCondStr.contains("blizzard") ) {
			condStr = "Snow";
			if (jsonCondStr.contains("thunder")) {
				condStr += "|Storm";
			} else if (jsonCondStr.contains("moderate")) {
	 ...
	 * @param requestSuffix - the request suffix string
	 ...
	 * @param conn - the HttpUrlConnection object to be used
	 * @throws IOException
	 */
	 ...
	 * @param urlConn - the HttpUrlConnection object to be used
