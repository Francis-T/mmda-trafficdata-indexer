...
/**
 * <b>FileSystemInterface Class</b>
 * </br>
 * Handles all interactions involving the various files used by the program and the filesystem 
 * itself
 * @author francis
 * 
 */
public class FileSystemInterface {
	 ...
	 * @param tagsetFileName - the name of the tag file to use
	 * @param dateStr - the date to extract a tag for
	 * @return a String containing the extracted tag
	 * @throws IOException
	 */
	public String getTagForDate(String tagsetFileName, String dateStr) throws IOException {
		...
		try {
			...
			while ((line = rd.readLine()) != null) {
				...
				/* Normally, the length of the line should be 2. Otherwise, we might be
				 * dealing with a malformed line. Therefore, we should skip it. 
				 */
				if (lineStr.length != 2) {
					continue;
				}
				
				/* Check if we've encountered the date whose tags we 
				 * want to retrieve */
	...
	/**
	 * Loads the tagset file and parses the stored tag info into a List of TagInfo objects.
	 * @param tagsetFileName - the name of the tag file
	 * @return a List of TagInfo objects
	 * @throws IOException
	 */
	public List<TagInfo> loadTagsetFile(String tagsetFileName) throws IOException {
		File tagsFile = new File(tagsetFileName);
		...
		String line = "";
		// Wrap a BufferedReader around the InputStream
		BufferedReader rd = new BufferedReader(new InputStreamReader(tagsFileStream));
	 ...
	 * @param tagFileName - the name of the tag file
	 * @param tagInfoList - a List of TagInfo objects containing the tags to be written to the 
	 * 						tag file
	 * @return an integer indicating the exit status for this method
	 * @throws IOException
	 */
	public int saveTagFile(String tagFileName, 
						   List<TagInfo> tagInfoList) throws IOException {
		...
		try {
			/* Initialize the file output stream to append to
			 * the existing partFile */
	...
	/**
	 * Checks the usability of the data in a part file by counting the number of entries currently 
	 * recorded in the currently-running Traffic Data Part File. This is mostly used to verify if 
	 * we have enough data to describe the traffic conditions for this day.
	 * @param partFileName - the name of the Part File 
	 * @return an integer indicating the number of distinct entries in the Part File
	 * @throws IOException
	 */
	public int checkPartFileTimestamps(String partFileName) throws IOException {
		/* We're going to base the sanity of the part file's data on the 
		 * number of complete records contained within it. */
		int recordCount = 0;
		File partFile = new File(partFileName);

		if (!partFile.exists()) {
			System.out.println("[checkPartFileTimestamps] ERROR: Part File Not Found!");
			return recordCount;
		}
		...
		try {
			...
			while((line = rd.readLine()) != null) {
				...
				/* If the new timestamp value would be different
				 * from the previous one, increase the record 
				 * counter by one. */
	...
	/**
	 * Extracts header information from a Historical Data File without actually extracting any of 
	 * the content data
	 * @param histDataFileName - the name of the Hist Data File
	 * @param isCompressed - a boolean indicating whether the Hist Data File is in DEFLATER
	 * 						 compressed format
	 * @return a HistDataFileInfo object containing info about the Hist Data File
	 */
	public HistDataFileInfo getHistDataFileInfo(String histDataFileName, boolean isCompressed) {
	...
	/**
	 * Loads the content data for a particular tag from the given Historical Data file
	 * @param histDataFileName - the name of the Hist Data file
	 * @param targetTagStr - the tag whose contents should be loaded from the Hist Data file
	 * @param isCompressed - a boolean indicating whether the Hist Data File is in DEFLATER
	 * 						 compressed format
	 * @return a HistData object containing the list of Line Data for the specified tag
	 * @throws IOException
	 */
...
//				System.out.println("Decompressed Out: " + line);
			...
			if (histData == null) {
				System.out.println("[loadHistDataTagFromFile] Warning: Tag not found! (" + 
																			targetTagStr + ")");
			}
		} catch (IOException e) {
			e.printStackTrace();
	...
	/**
	 * Loads the all content data from the specified Hist Data File into a (supposedly) empty List 
	 * of HistData objects
	 * @param histDataFileName - the name of the Hist Data file
	 * @param histDataList - an empty list of HistData objects; this is where the information
	 * 						retrieved from the Hist Data file will be stored.
	 * @param isCompressed - a boolean indicating whether the
	 * 						 Hist Data File is in DEFLATER
	 * 						 compressed format
	 * @throws IOException
	 */
...
//				System.out.println("Decompressed Out: " + line);
	...
	/**
	 * Prepares the HistData file to be written on by writing the relevant header information to the 
	 * file first prior to allowing any Historical Data content to be written.
	 * @param histDataFileName - the name of the Hist Data File
	 * @param fileInfo - a HistDataFileInfo object containing the HistData file header information
	 * 					to be written
	 * @param shouldCompress - a boolean indicating whether the Hist Data File is in DEFLATER
	 * @throws IOException
	 */
	public void prepareHistDataFile(String histDataFileName, 
									HistDataFileInfo fileInfo, 
		...
		if (fileInfo == null) {
			System.out.println("[prepareHistDataFile] ERROR: HistDataFileInfo is NULL!");
			return;
		}
		
		if ((fileInfo.tagList == null) || (fileInfo.tagList.size() == 0)) {
			System.out.println("[prepareHistDataFile] ERROR: There seem to be no new tags to " + 
																			"add to the file.");
			return;
		}
		
		try {
			HistDataFileInfo oldHistInfo = getHistDataFileInfo(histDataFileName, shouldCompress);
			...
			if (!saveFile.exists()) {
				...
				if (shouldCompress) {
					tempFile = createTempDecompressedFile("temp_hdf.txt", saveFile, false);
				} else {
					tempFile = createTempUncompressedFile("temp_hdf.txt", saveFile, false);
				}
			}
	...
	/**
	 * Loads information from the specified Raw Traffic Data File into a LineDataList
	 * @param dataFileName - the name of the Raw Traffic Data File to be used
	...
	/**
	 * Loads information from the specified Raw Traffic Data File into a LineDataList
	 * @param dataFile - a File object pertaining to the Raw Traffic Data File to be used
	 * @return a LineDataList containing Traffic Data
	 */
	...
	/**
	 * Loads information from a Traffic Part Data File into a List of LineInfo objects. 
	 * @param partFileName - the name of the Part File
	 * @return a List of LineInfo objects containing info from the Part File
	 * @throws IOException
	 */
	public List<LineInfo> loadPartDataFile(String partFileName) throws IOException {
		File partFile = new File(partFileName);
		List<LineInfo> dataList = new ArrayList<LineInfo>();

		if (!partFile.exists()) {
			System.out.println("[loadPartDataFile] ERROR: Part File Not Found!");
			return null;
		}
	 ...
	 * @param partFileName - the name of the Part File
	 * @param timestamp - the Timestamp for this Line Data
	 * @param lineDataStr - the Line Data String
	 * @return an integer indicating the exit status for this method
	 * @throws IOException
	 */
	public int saveDataToPartFile(String partFileName, 
								  String timestamp, 
	...
	/**
	 * Saves data from a List of LineInfo objects into a Hist Data File. This is mostly used for 
	 * updating a previously generated Hist Data File with a new day's data.
	 * @param histDataFileName - the name of the Hist Data File
	 * @param lineInfoList - the List of LineInfo objects containing information to be written to 
	 * 							the Hist Data File
	 * @param tagStr - the tag to associate with this List of Line Info objects
	 * @param shouldWriteTag - a boolean indicating whether we should write the tag as part of the
	 * 							content data
	 * @param shouldCompress - a boolean indicating whether the Hist Data File is in DEFLATER
	 * @throws IOException
	 */
	public void saveDataToFile(String histDataFileName, 
		...
		try {
			...
			/* Need to inflate the existing file's contents to an external file first if we
			 * want to append to the compressed file */
			if (shouldCompress) {
				tempFile = createTempDecompressedFile("temp.txt", saveFile, true);
			} else {
				tempFile = createTempUncompressedFile("temp.txt", saveFile, true);
			}
			...
			if (shouldCompress) {
				...
				/* Write data back from our tempFile to our saveFile using the writer */
			...
			while(iter.hasNext()) {
				...
				wr.write(tempLineInfoObj.timestamp + ":" + tempLineInfoObj.lineDataStr);
				wr.newLine();
				wr.flush();
			}
	...
	/*****************************************************************************************/
	/**
	 * Creates a temporary decompressed file where we will store header and content info from the
	 * intermediate Hist Data file while writing in new data.
	 *</br></br>
	 *<b>NOTE:</b> There might be a better way of achieving this effect by simply using the
	 *				underlying filesystem's file "copy" command. However, since we do not 
	 *				necessarily know what system we will be running in, this workaround was
	 *				used instead.
	 * @param tempFileName - the name of the Temp File
	 * @param originalFile - the name of the Original Compressed HistData file
	 * @param shouldCopyHeader - a boolean indicating whether we should copy the header info
	 * @return a File object referring to the created Temp File
	 * @throws IOException
	 */
	private File createTempDecompressedFile(String tempFileName, 
											File originalFile, 
											boolean shouldCopyHeader) throws IOException {
	...
	/**
	 * 
	 * Creates a temporary file where we will store header and content info from the intermediate 
	 * Hist Data file while writing in new data. This method specifically targets uncompressed
	 * Hist Data files.
	 *</br></br>
	 *<b>NOTE:</b> There might be a better way of achieving this effect by simply using the
	 *				underlying filesystem's file "copy" command. However, since we do not 
	 ...
	 *				used instead.
	 * @param tempFileName - the name of the Temp File
	 * @param originalFile - the name of the Original HistData file
	 * @param shouldCopyHeader - a boolean indicating whether we should copy the header info
	 * @return a File object referring to the created Temp File
	 */
	...
	/**
	 * Writes the contents of the generated Hist Data temporary file back to the intermediate
	 * Hist Data file while skipping the particular tags which have just been updated.
	 * @param tempFile - the name of the Temp File
	 * @param writeTarget - BufferedWriter object to use for writing
	 * @param tagSkipStr - a String specifying which tag should be skipped
	 * @throws IOException
	 */
	private void writeInputFromTempFile(File tempFile, 
										BufferedWriter writeTarget, 
										String tagSkipStr) throws IOException {
		if (!tempFile.exists()) {
			return;
		}
		...
		try {
			...
			while ((line = rd.readLine()) != null) {
				...
				if ((line.length() > 0) && (line.charAt(0) == '>')) {
					String lineTag = line.substring(line.indexOf('[')+1, line.lastIndexOf(']'));
					
					System.out.println("skip str: [" + tagSkipStr + "]");
					System.out.println("line tag: [" + lineTag + "]");
					
					if ((tagSkipStr != null) && (lineTag.equals(tagSkipStr))) {
						System.out.println("[writeInputFromTempFile] Skipping [" + tagSkipStr + "]...");
						isSkipping = true;
					} else {
						isSkipping = false;
					}
	 ...
	 * @param dataFile - a File object pertaining to the Raw Traffic Data File to be used
	 * @return a LineDataList object containing Traffic Data information for each line
	 * @throws IOException
	 */
	private LineDataList parseTrafficFileContents(File dataFile) throws IOException {
		...
		while ((line = rd.readLine()) != null) {
			...
			if (line.split(",").length != 5) {
				System.out.println("[parseTrafficFileContents] Invalid line length encountered: " + line.split(",").length + "!");
				continue;
			}
			
			tempLocStr = (getCSVField(line, 0, lineNum) + ", " + getCSVField(line, 1, lineNum)).replace("��", "n").replace("ñ", "n");
			tempLoc = matchToLocIndex(tempLocStr);
			if ((tempLoc > Constants.locStr.length) || (tempLoc < 0)) {
				System.out.println("[parseTrafficFileContents] Error: Invalid location code: " + tempLoc + "!");
				System.out.println("[parseTrafficFileContents] 		> LocStr: " + tempLocStr + "!");
				System.out.println("[parseTrafficFileContents] 		> Line: " + line + "!");
				continue;
			}
			...
			if ((tempLoc - prevLoc) > 1) {
				while ((tempLoc - prevLoc) != 0) {
					/* Pre-increment the value of prevLoc (since we generally start this at -1) */
					prevLoc++;
		...
		/* Check if the final LineDataList is empty.
		 * - This would be the case if the file were empty in the first place 
		 */
		if (ld.data.size() == 0) {
			return null;
		}
	 ...
	 * @param matchLocStr - the location string to match
	...
	/**
	 * Gets a particular CSV field from a given line. This method is mostly
	 * used for reading CSV files.
	 * @param line - line containing CSV fields
	 * @param field - index of the CSV field to obtain
	 * @param lineNum - (Optional) used to indicate the line number for this
	 * 					line in the parsed CSV file
