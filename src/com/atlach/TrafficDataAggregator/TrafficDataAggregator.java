...
/**
 * <b>TrafficDataAggregator Class</b>
 * </br>Handles the aggregation of Traffic Data from generated Raw Traffic Data Files.
 * Functionality is provided for the following cases:
 * </br>- (Re)generating Historical Data from a directory of Raw Traffic Data Files
 * </br>- Updating Historical Data from a directory of Raw Traffic Data Files
 * </br>- Push Traffic Data from a Raw Traffic Data File into a Traffic Data Part File.
 * </br>- Offload the data in a Part File into a Historical Data File
 * @author francis
 *
 */
public class TrafficDataAggregator {
	...
	public TrafficDataAggregator(String histDataFile, String tagFile, boolean shouldCompress) {
	...
	/*****************************************************************************************/
	/**
	 * Generates a new HistData file given the Traffic Data files stored in the directory
	 * specified by targetDir
	 * @param targetDir - the name of the directory where the Traffic Data files are stored
	...
	public int generateHistDataFile(String targetDir) {
		...
		/* Attempt to obtain information about the pre-existing HistData file if it exists */
		String datesCovered = "";
		...
		/* Invoke the parsing of all non-aggregated traffic data files in the target dir */
		result = dfm.parseTrafficDataFiles(targetDir, masterList, datesCovered);
		if (result != STATUS_OK) {
			System.out.println("[generateHistDataFile] Failed to parse traffic data files!");
			return STATUS_FAILED;
		}
		
		/* Normalize the line data strings for all HistData objects in the masterList */
		/* NOTE: This is done since we want to condense the traffic data bytes into 
		 * 		 their rough average since we can store up to 4 instances of traffic
		 * 		 info for each particular "line"/"location" per byte.
		 */
		result = dfm.normalizeHistDataList(masterList);
		if (result != STATUS_OK) {
			System.out.println("[generateHistDataFile] Failed to normalize HistData list!");
			return STATUS_FAILED;
		}
		
		/* Since our initial list is essentially date-based, we will have to do a second
		 * sorting in order to create the Tag-based list. In this step, we will merge
		 * together the information within HistData objects which have the same set of
		 * tags. This simply means that multiple dates may be merged in a single tagset. */
		...
		datesCovered = updateDateRangeString(datesCovered, dfm.getDatesCoveredString());
		
		/* Reuse the HistDataFileInfo object from before */
		hdInfo = new HistDataFileInfo(datesCovered, parsedTagList);
		/* TODO: fsi.getDatesCoveredString() may be too reliant on parseTrafficDataFiles() 
		...
		try {
			...
			/* Cycle through each HistData object in the tagCentricList and save the 
			 * information to the specified HistData file */
			int i = 0;
			for (i = 0; i < tagCentricList.size(); i++) {
				HistData hd = tagCentricList.get(i);
				fsi.saveDataToFile(histDataFileName, hd.dataList, hd.tagset, true, useCompression);
			}
			System.out.println("INFO: A total of " + i + " elements were changed.");
		} catch (IOException e) {
			e.printStackTrace();
			return STATUS_FAILED;
		}

		System.out.println("[generateHistDataFile] Finished Successfully (for Dir).");
		return STATUS_OK;
	}
	
	/**
	 * Updates an existing HistData file with new Traffic Data from the specified directory. 
	 * @param refList - a reference HistData object list containing existing Historical Data
	 * @param targetDir - the name of the directory where the Traffic Data files are stored
	 * @return an integer indicating the exit status for this method
	 */
	public int updateHistDataFile(List<HistData> refList,
								   String targetDir) {
		...
		if (refList == null) {
			/* TODO We might try incorporating reference list generation here as well */
			System.out.println("ERROR: Cannot perform updates on an empty reference list!");
			return STATUS_FAILED;
		}
		...
		/* Reuse the HistDataFileInfo object from before */
		hdInfo = new HistDataFileInfo(datesCovered, parsedTagList);
		/* TODO: fsi.getDatesCoveredString() may be too reliant on parseHistDataFiles() 
		 * 		 being called before it... */
		...
		try {
			...
			for (int i = 0; i < refList.size(); i++) {
				if (refList.get(i).hasChanged == false) {
					...
					System.out.println("> Incorporating changes for: " + refList.get(i).tagset);
				}
			...
			System.out.println("INFO: A total of " + c + " elements were changed.");
		} catch (IOException e) {
			e.printStackTrace();
			return STATUS_FAILED;
		}

		System.out.println("[updateHistDataFile] Finished Successfully (for Dir).");
		return STATUS_OK;
	}
	 ...
	 * </br>
	 * <i>(This is mostly used for continuously updating the running Part File 
	 * with intermediate Traffic Data as the day progresses)</i>
	 * @param partFileName - the name of the Part File
	 * @param partFileTag - the tag to use for this Part File
	 * @param partFileDate - the date associated with this Part File
	 * @return an integer indicating the exit status for this method
	 */
	public int updateHistDataFile(String partFileName,
								   String partFileTag,
								   String partFileDate) {
		...
		/* Attempt to obtain information about the pre-existing HistData file if it exists 
		 * NOTE: It SHOULD exist if we're going to update it with a _part_ */
		String datesCovered = "";
		hdInfo = fsi.getHistDataFileInfo(histDataFileName, useCompression);
		if (hdInfo != null) {
			datesCovered = hdInfo.datesCovered;
		...
		try {
			...
			if (newHistData.dataList == null) {
				System.out.println("[updateHistDataFile] ERROR: Failed to load specified" +
																			" Part File!");
				return STATUS_FAILED;
			}
			...
			/* If this tag is not distinct, then we must merge it with old historical data */
			if (!isDistinctTag) {
				...
				if (oldHistData == null) {
					System.out.println("[updateHistDataFile] ERROR: Failed to load Tag from " + 
																			"Hist Data File!");
					return STATUS_FAILED;
				}
			...
			fsi.saveDataToFile(histDataFileName, 
							   newHistData.dataList, 
							   newHistData.tagset, 
							   true, 
							   useCompression);
		} catch (IOException e) {
		...
		System.out.println("[updateHistDataFile] Finished Successfully (for Part File).");
		return STATUS_OK;
	}
	
	/**
	 * Pushes Traffic Data from the regular-format .csv file produced by TrafficDataIndexer to
	 * a Part File maintained by the TrafficDataAggregator. This is the compact version of this
	 * method, filling in the name of the Part File with the default Part File name.
	 * </br>
	 * <i>(This is mostly used for continuously updating the running Part File with intermediate 
	 * Traffic Data as the day progresses.)</i>
	 * @param trafficDataFileName - the name of the target Traffic Data file produced by
	 * 								TrafficDataIndexer 
	 * @param timestamp - the time to associate with this data part 
	 * @param dataTag - the tag to associate with this data part
	 * @return an integer indicating the exit status for this method
	 */
	public int pushTrafficData(String trafficDataFileName, String timestamp, String dataTag) {
		return pushTrafficData(trafficDataFileName, timestamp, dataTag, defaultPartFileName);
	}

	/**
	 * Pushes Traffic Data from the regular-format .csv file produced by TrafficDataIndexer to
	 * a Part File maintained by the TrafficDataAggregator. This is the full version of this
	 * method, having the Part File name as an additional argument
	 * </br>
	 * <i>(This is mostly used for continuously updating the running Part File with intermediate 
	 * Traffic Data as the day progresses.)</i>
	 * @param trafficDataFileName - the name of the target Traffic Data file produced by
	 ...
	 * @param timestamp - the time to associate with this data part 
	 * @param dataTag - the tag to associate with this data part
	 * @param partFileName - the name of the target the Part File
	 * @return an integer indicating the exit status for this method
	 */
	public int pushTrafficData(String trafficDataFileName, 
							   String timestamp, 
							   String dataTag, 
							   String partFileName) {
		...
		System.out.println("[pushTrafficData] Checking if tag file should be updated " +
											"(timestamp = " + timestamp + ")...");
		if (((hour % 6) - 2) == 0) {
			System.out.println("[pushTrafficData] Conditions are right. Updating tag file...");
			result = pushTagFileUpdate();
			if (result != STATUS_OK) {
				System.out.println("[pushTrafficData] Error: Failed to update the tag file!");
			}
		...
		if (hour == 0) {
			System.out.println("[pushTrafficData] Attempting to offload previous day's data...");
			result = offloadCollectedData(partFileName, "", fsi);
			if (result != STATUS_OK) {
				System.out.println("[pushTrafficData] Error: Failed to offload data!");
				return STATUS_FAILED;
			}
		...
		System.out.println("[pushTrafficData] Finished. Exit Status: " + result);
		return result;
	}
	
	/**
	 * Offloads data collected in the Part File into the HistData file. This is the compact
	 * version of this method, disallowing the option to force offloading of partial data.
	 * @param partFileName - the name of the Part File to offload into the HistData file
	 * @param dataTag - the tag to associate with this Part File's data
	 * @param fsi - the FileSystemInterface object to use for the offload transaction
	 * 				TODO: This may no longer be necessary since we already instantiated a
	 * 					FileSystemInterface object when the TrafficDataAggregator was 
	 * 					instantiated.
	 * @return an integer indicating the exit status for this method
	 */
	public int offloadCollectedData(String partFileName, String dataTag, FileSystemInterface fsi) {
		return offloadCollectedData(partFileName, dataTag, fsi, false);
	}
	
	/**
	 * Offloads data collected in the Part File into the HistData file. This is the full
	 * version of this method, allowing the option to force offloading of partial data.
	 * @param partFileName - the name of the Part File to offload into the HistData file
	 * @param dataTag - the tag to associate with this Part File's data
	 * @param fsi - the FileSystemInterface object to use for the offload transaction
	 * 				TODO: This may no longer be necessary since we already instantiated a
	 * 					FileSystemInterface object when the TrafficDataAggregator was 
	 * 					instantiated.
	 * @param forceOffload - a boolean indicating whether data offload should be forced
	 * @return an integer indicating the exit status for this method
	 */
	public int offloadCollectedData(String partFileName, 
									String dataTag, 
		...
		if (!forceOffload) {
			/* Check whether it is sane for us to offload data from the part file */
			try {
			...
			/* i.e. If we have less than 16 (2/3rds of the day) complete records, 
			 * 		then we might not have sufficient collected data to offload */
			if (checkResult < 16) {
				System.out.println("[offloadCollectedData] Error: Not enough data to " + 
																 "offload (" + checkResult + ")!");
				return STATUS_FAILED;
			}
		}
		...
		if (dataTag.equals("")) {
			try {
				...
				if ((partFileTag == null) || (partFileTag.equals(""))) {
					System.out.println("[offloadCollectedData] Warning: No associated tags could" +
																					 " be found!");
					
					/* Use the default tagging scheme instead: 		*/
					/* i.e. <Day of Week Type>|<Day of Week Name> 	*/
					partFileTag = getDefaultFileTag();
					System.out.println("[offloadCollectedData] Using default tag: " + partFileTag);
				}
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		System.out.println("[offloadCollectedData] Updating HistData file with previous day's" +
																		  " Part File data...");
		...
		System.out.println("[offloadCollectedData] Finished. Exit Status: " + result);
		return result;
	}
	
	/**
	 * Updates the tag file used by the Aggregator with the current day's tag inferred from the
	 * current date and the current weather conditions.
	 * @return an integer indicating the exit status for this method
	 */
	public int pushTagFileUpdate() {
		...
		if (i == tagInfoList.size()) {
			...
			tagInfoList.add(new TagInfo(todayStr, (defTag + ", " + weatherTag)));
		} else {
			...
			if (weatherTag.contains("Rain")) {
				if (currentTag.contains("Rain")) {
					if (currentTag.contains("Moderate") && 
						(weatherTag.contains("Heavy") || weatherTag.contains("Storm"))) 
					{
						newWeatherTag = weatherTag;
					} else if (currentTag.contains("Heavy") && weatherTag.contains("Storm")) {
						newWeatherTag = weatherTag;
					}
		...
		try {
			...
			if (result != STATUS_OK) {
				System.out.println("[pushTagFileUpdate] Failed to save tag file!");
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
		
		System.out.println("[pushTagFileUpdate] Finished. Exit Status: " + result);
		return result;
	}
	...
	/** PRIVATE FUNCTIONS																	**/
	/*****************************************************************************************/
	private static final String dayOfWeekStr[] = { "Unknown-day", 
												   "Sunday", 
												   "Monday", 
												   "Tuesday", 
												   "Wednesday", 
	...
	/**
	 * Obtains a tag describing the current weather conditions according to data from
	...
	/**
	 * Attempts to get the default tag to be used with the current Part Data file to be offloaded.
	 * @return a tag string
	 */
	...
	/**
	 * Gets the date of the previous day. This is mostly used during Part Data File offload.
	...
	/**
	 * Updates the given date range string with a new date range. This is mostly used to update
	 * the "DatesCovered" tag used in the header of Historical Data Files.
	 * @param oldDateRange
	 * @param newDateRange
	 * @return
	 */
	private String updateDateRangeString(String oldDateRange, String newDateRange) {
		String dateRangeStr = "";
		
		/* Combine the oldDateRange and newDateRange strings, and then deconstruct
		 * the result into a string array */
		...
		for (int i = 0; i < dateRangeArr.length; i++) {
			...
			if (dateRangeArr[i].contains("-")) {
				...
				for (int j = 0; j < dateRangeArr.length; j++) {
					...
					String result[] = adjustDateRange(dateRangeArr[j], rangeStr, highRange, lowRange);
				...
				for (int j = 0; j < dateRangeArr.length; j++) {
					...
					if (dateRangeArr[j].contains("-")) {
						...
						if (dateVal == highRange+1) {
							dateRangeArr[j] = rangeStr[0] + "-" + dateRangeArr[i];
							dateRangeArr[i] = "";
							break;
						}
						
						if (dateVal == lowRange+1) {
							dateRangeArr[j] = dateRangeArr[i] + "-" + rangeStr[1];
							dateRangeArr[i] = "";
							break;
						}
		...
		return (dateRangeStr.charAt(dateRangeStr.length()-1) != ',' ? dateRangeStr : dateRangeStr.substring(0, dateRangeStr.length()-1));
	}
	
	/**
	 * Adjusts an intermediate date range string by attempting to merge it with another date range
	 * string. Forgot exactly how this works...
	 * @param compStr - the date range string which we will attempt to merge to the target date
	 * 					range string
	 * @param rangeStr - an array containing info about the date range string to merge to
	 * @param highRange - the upper range of the target date range string
	 * @param lowRange - the lower range of the target date range string
	 * @return
	 */
	private String[] adjustDateRange(String compStr, String rangeStr[], int highRange, int lowRange) {
	...
	/**
	 * Creates a List of HistData objects classified according to their tags instead of dates.
	 * This method attempts to merge together HistData objects with different dates but having
	 * the same tag sets.
	 * @param tagCentricList - an empty List of HistData objects which will be classified by tag
	 * @param baseList - the original List of HistData objects classified by date
	 */
	private void createTagCentricList(List<HistData> tagCentricList, List<HistData> baseList) {
		if ((baseList == null) || (baseList.size() == 0)) {
			System.out.println("[createTagCentricList] ERROR: Invalid baseList!");
			return;
		}
		...
		for (int i = 0; i < baseList.size(); i++) {
			...
			for (int j = 0; j < tagCentricList.size(); j++) {
				...
				if (insTagset.equals(compTagset)) {
					...
					hasMerged = true;
					System.out.println("Set to hasChanged: " + tagCentricList.get(j).tagset + "/" + tagCentricList.get(j).date);
				}
			}
			if (!hasMerged) {
				...
				System.out.println("Set to hasChanged: " + hd.tagset + "/" + hd.date);
				hd.hasChanged = true;
				hd.date = "UNKNOWN";
	 ...
	 * @param dataList - the List of HistData objects to be used as reference
...
//
//		HistDataFileInfo hdInfo = fsi.getHistDataFileInfo("TrafficData.hist", false);
//		if (hdInfo != null) {
//			hdInfo.printInfo();
//		}
//		
...
//				
//				if ((hd.tagset.contains("Weekday|Wednesday, Weather|Clear|Hot")) && (li.timestamp.equals("1200"))) {
//					/* System.out.println(">" + li.timestamp + ":" + FileSystemInterface.decompressString(li.lineDataStr));*/
...
//		}
//		System.out.println("========================================================================================================");
//		result = tda.updateHistDataFile("TrafficData.part", "Weekday|Wednesday, Weather|Clear|Hot", "20130926", true);
//		if (result != STATUS_OK) {
//			return;
//		}
