...
/**
 * <b>TrafficDataAggregatorMain Class</b> </br>Handles the running of Push
 * Traffic Data and Generate Historical Data tasks using an instance of the
 * TrafficDataAggregator class
 * 
 * @author francis
 * 
 */
public class TrafficDataAggregatorMain implements TrafficDataIndexerNotifier {
	 ...
	 * 
	 * @param filename
	 *            - the name of the Raw Traffic Data File to push
	 * @param timestamp
	 *            - the timestamp to associate with the data
	 */
	public void runPushDataTask(String filename, String timestamp) {
		if (aggregatorPushDataTask == null) {
			aggregatorPushDataTask = new PushTrafficDataTask(filename,
					timestamp, this);
	...
	/**
	 * Runs (Re)generate Historical Data using the TrafficDataAggregator through
	 * a thread
	 * 
	 * @param directory
	 *            - the directory containing Raw Traffic Data Files to be used
	 *            to generate the Hist Data File
	 */
	public void runGenerateHistDataTask(String directory) {
		if (aggregatorGenHistDataTask == null) {
			...
			if (aggregatorGenHistDataThread == null) {
				aggregatorGenHistDataThread = new Thread(
						aggregatorGenHistDataTask);
			}
		...
		public void run() {
			if (directory != null) {
				...
				if (result != TrafficDataAggregator.STATUS_OK) {
					System.out
							.println("[GenHistDataTask] Generate Hist Data File Failed!");
				}
			} else {
	...
	class PushTrafficDataTask implements Runnable {
		...
		public PushTrafficDataTask(String filename, String timestamp,
				TrafficDataIndexerNotifier e) {
			tda = new TrafficDataAggregator();
		...
		public void run() {
			...
			// int result = tda.offloadCollectedData("TrafficData.part",
			// "Weekend|Saturday, Weather|Rain|Moderate", new
			// FileSystemInterface(), true);
			if (result != TrafficDataAggregator.STATUS_OK) {
	...
	/**
	 * Waits for the operation of the Push Traffic Data and Generate Hist Data
	 * tasks to stop. </br>NOTE: It may cause lots of problems if we stop this
	 * prematurely, especially when we're dealing with HistData files which we
	 * can't readily recreate.
	 */
	public void stopOperation() {
		if (isRunning) {
			try {
				System.out
						.println("Waiting for push traffic data task to finish...");
